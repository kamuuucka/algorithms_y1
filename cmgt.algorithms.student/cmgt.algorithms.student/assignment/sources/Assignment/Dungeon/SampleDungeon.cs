...
class SampleDungeon : Dungeon
{
	...
	protected override void generate(int pMinimumRoomSize)
	{
		...
		roomA = new Room(new Rectangle(0, 0, size.Width / 2 + 1, size.Height));
			rooms.Add(roomA);
		//right room from half of screen to the end
		roomB = new Room(new Rectangle(size.Width / 2, 0, size.Width / 2, size.Height));
		rooms.Add(roomB);
		...
		doors.Add(new Door(new Point(size.Width / 2, size.Height / 2 + Utils.Random(-5, 5)), roomA, roomB));
	}
}
